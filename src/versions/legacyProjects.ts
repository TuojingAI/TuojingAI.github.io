...
export const projects: Project[] = [
...

  {
    ...
    body: [
      ...
      {
        kind: "p",
        text: "论文自陈了几项范围限制。全部结果都在仿真中取得，因为仅评测可见的 FEM 状态才使形变可直接测量；当前任务属于同一个 pick-and-place 技能族；每个配置只训练一次，跨策略族的差异可能同时反映架构与各自规定的训练配方。触觉方面，SoftVTBench 沿用 TacEx 的 GelSight Mini 渲染 profile 与 Taxim 的示例式光学标定，没有做面向本基准的真实传感器标定，也没有做任何真机对比；Taxim 与 FOTS 在各自原论文中的验证不构成对 SoftVTBench 具体资产、材料、接触条件与渲染参数的 sim-to-real 验证。因此本工作的结论限定在仿真视触觉策略学习范围内，模拟触觉与物理触觉之间的差距未被刻画。后续工作指向更广的操作技能、更多资产、重复训练与真机部署。",
      },
    ],
  },
