...
import kgdegeneracy from "../assets/projects/knockgs-degeneracy.webp";
import kgidentifiability from "../assets/projects/knockgs-identifiability.webp";
import kgobjects from "../assets/projects/knockgs-objects.webp";
import kgoverview from "../assets/projects/knockgs-overview.webp";
...
const ALL_PROJECTS: Project[] = [
  {
    ...
    date: "2026 年 8 月",
    authors:
      "Chenchen Ge*, Hanwen Shen*, Bowen Jing, Jiyuan Cai, Xiaofeng Wang, Hongsen Lei, Weitao Zhou, Dandan Zhang, Haibao Yu†（* equal contribution　† corresponding author）",
    affiliations:
    ...
    body: [
      ...
      { kind: "p", text: "论文指出，质量与刚度差别很大的两个物体在重力下可以掉得很像，外观几乎相同的物体对同一次物理交互可以有相当不同的响应。它把这归为被动观测模型的可辨识性问题，并给出了一个可以核对的结构原因：在 ReconPhys 所用的弹簧—质点公式里，公开发布的实现算出的弹簧力与阻尼力对 k 和 d 是线性的，重力项是 m·g，积分为 v ← v + FΔt/m，而该配置用一个与质量无关的运动学投影处理地面接触。于是重缩放 (m, k, d) → (αm, αk, αd) 不改变任何一个粒子的轨迹，也不改变任何一帧渲染。被动下落条件下只有 k/m 这类比值可辨识，绝对质量不可观测。论文说明这来自观测设定与参数化而非实现错误，并举例说依赖质量的罚函数接触模型有可能破坏这个缩放不变性。" },
      { kind: "p", text: "在五个留出目标上核对的结果：真实比值 s_E/s_ρ 的跨度是 162%，KnockGS 估计的跨度是 163%，公开发布的 ReconPhys 在 k/m 上只跨 4.5%；PhysGM 因为四个静态视角在五个目标上完全相同，只返回一个预测。论文注明这三个跨度是不同参数化下的量程比较，不是同口径的参数精度对比。ReconPhys 的原生输出还贴在它公布的训练区间上沿：质量落在 [0.2, 6.0] 里的 5.55–5.88，阻尼落在 [0.1, 5.0] 里的 4.83–4.97，摩擦在全部五个目标上为负，尽管该方法定义 f ≥ 0。" },
      { kind: "fig", fig: { src: kgidentifiability, caption: "五个留出目标上的被动观测诊断（a）：各序列按自身均值归一化后的可分辨范围。论文注明该图诊断的是不同参数化下的量程与饱和，不是同口径的参数精度比较。" } },
      { kind: "fig", fig: { src: kgdegeneracy, caption: "同一诊断的（b）：公开发布的 ReconPhys 实现的原生输出相对其公布的训练区间，叉号表示不可接受的取值。" } },
      { kind: "h", text: "任务定义" },
      { kind: "p", text: "论文把目标问题写成：给定初始观测 O₀、一次已知交互 u_A 和可观测的响应 Y_A，推断一个表示 z_phys，用它预测另一次不相交的交互 u_B 下的响应 Y_B。判据不是参数数值接近，而是冻结后的表示能否预测 u_B 下的运动、形变、接触，直到渲染出来的视觉响应。" },
      { kind: "p", text: "实现上分成两个探针。标定探针 A 用来估参数；预测探针 B 在方向、幅值或两者上不同，估计过程中从不出现，参数在 B 施加之前已冻结。Pillow 的 A 是 standard_x，对全部 695,984 个物理状态粒子施加力矢量 (−0.18, 0, 0)，作用 1 个 MPM 步即 0.1 ms；B 有两个，换方向的 standard_y 力为 (0, −0.18, 0)，加幅值的 strong_x 力为 (−0.36, 0, 0)。Vasedeck 的 A 与 B 是限定在接触盒内的 standard_x 与 standard_z，力分别为 (−0.09, 0, 0) 和 (0, 0, −0.09)，选中 4,464 个粒子。方向与幅值分开变，用于把迁移失败归因到其中一个。" },
      { kind: "fig", fig: { src: kgoverview, caption: "KnockGS 及其冻结预测协议的总览。设置部分固定可仿真的高斯资产 G 与仿真契约 C。(1) 已知的探针 A 产生被观测的目标响应 R_A*。(2) 同一个确定性五维描述子 φ 编码该响应以及预先算好的物体特定库 D_A 中的全部响应；只用库自身统计量做标准化、硬性取前 10 近邻、等权局部岭拟合，得到连续的弹性与密度尺度 (s_E, s_ρ)。(3) 估计值被冻结并写回同一个仿真器，用于预测留出探针 B 下的 R̂_B。目标 R_B* 只在冻结之后揭晓，轨迹 RMSE 是主要证据。" } },
      { kind: "h", text: "方法" },
      {
        kind: "list",
        items: [
          { term: "离线响应库", desc: "每个物体一套，只建一次。54 个预先声明的 (s_E, s_ρ) 候选对，不是笛卡尔网格；s_E ∈ {0.50, 0.55, 0.65, 0.75, 0.85, 0.95, 1.00, 1.05, 1.15}，s_ρ ∈ {0.75, 1.00, 1.10, 1.20, 1.25, 1.35, 1.50, 1.65, 1.75}。每个候选跑一次 MPM，内部子步 10⁻⁴ s，导出 31 个状态（含初始态）。所有留出目标都被排除在库外。" },
          { term: "五维响应描述子", desc: "先用初始粒子包围盒的对角线归一化，算出 RMS 位移曲线 r^t 与帧差速度曲线 v^t，再取 [AUC(r), 早期形变速率, max_t v^t, AUC(v), 前 5 帧速度占全程之比]。目标响应与每个候选响应过同一个映射。没有可学习的编码器，标准化只用候选库自身的统计量。论文说明这个描述子不被假定为让两个尺度全局可辨识，它只提供局部证据。" },
          { term: "硬邻域局部岭回归", desc: "欧氏距离只用来选出最近的 10 个候选，组内等权，α = 10⁻³ 的岭回归拟合出连续的 (s_E, s_ρ)。截距不做正则，两个尺度独立回归，预测裁剪到候选集边界。只有在可用邻居少于三个或岭求解出数值异常时才回退到反距离加权 KNN。k = 10、α = 10⁻³ 在任何留出评测之前已固定。不对 MPM 求导，标定一个新目标不需要新的仿真。" },
          { term: "冻结写回与留出预测", desc: "估计值立即冻结，写回生成响应库的同一个仿真器。先用 A 重放作为一致性诊断，再在 B 上预测。B 的真值 R_B* 只在最后比较时揭晓。主要的动力学比较是有稳定身份的高斯关联粒子上的轨迹 RMSE，Pillow 上最多抽 100,000 个、固定种子 42。" },
        ],
      },
      { kind: "p", text: "被标定的只有两个无量纲尺度 (s_E, s_ρ)。论文写明它们是相对该物体固定基准模板的有效标定尺度，不是内禀材料常数的直接估计：Pillow 的基准是 E = 10,000、ρ = 2,000、ν = 0.30，Ficus 是 E = 2×10⁶、ρ = 200、ν = 0.40，Vasedeck 是 E = 10,000、ρ = 40、ν = 0.30。其余一切被契约冻住 —— 资产、坐标变换、内部填充构造、MPM 网格、本构族、接触模型、边界条件、阻尼、摩擦、泊松比、相机、渲染器。高斯的粒子式结构直接映射到 MPM 物质点，不需要显式网格；外观与力学状态留在同一个资产上；成熟的 3DGS 重建管线因此可以提供可仿真资产的几何与视觉基础。主轨迹指标只在高斯关联粒子上计算，因为内部填充粒子不可渲染，且改变填充构造会改变它们的数量与对应关系。Pillow 的 695,984 个物理状态粒子里，624,324 个是高斯关联粒子，71,660 个是内部填充粒子。" },
      { kind: "h", text: "主结果" },
      { kind: "p", text: "主基准是重建出来的 pillow/sofa 高斯资产，网格 n_grid = 100，模拟 0.60 s。五个留出目标 (0.60, 1.65)、(0.70, 1.20)、(0.80, 1.60)、(0.95, 1.35)、(1.05, 1.10) 落在覆盖域内，但都不是 54 个候选中的任何一对。所有可部署方法拿到的信息完全相同：同一个响应库、同一个描述子、同一份 Probe-A 响应；固定默认材料按构造忽略这份证据；参数最近邻读取隐藏的目标位置，只作为 oracle 诊断报告。成本集中在离线且每个物体只付一次：建库需要 54 次 MPM rollout，之后标定一个新目标不增加任何 rollout。" },
      {
        kind: "table",
        head: ["方法", "联合参数误差 (%)", "Probe A 轨迹 RMSE", "Probe B 方向", "Probe B 幅值"],
        rows: [
          ["固定默认材料", "27.31", "3.682×10⁻³", "3.502×10⁻³", "4.407×10⁻³"],
          ["响应最近邻", "6.75", "3.722×10⁻⁴", "3.533×10⁻⁴", "5.188×10⁻⁴"],
          ["响应 KNN", "2.37", "1.449×10⁻⁴", "1.412×10⁻⁴", "2.020×10⁻⁴"],
          ["全局岭回归", "2.45", "1.748×10⁻⁴", "1.687×10⁻⁴", "2.466×10⁻⁴"],
          ["局部岭回归（本文）", "1.13", "4.498×10⁻⁵", "4.424×10⁻⁵", "7.786×10⁻⁵"],
          ["参数最近邻 oracle", "4.45", "6.283×10⁻⁴", "5.984×10⁻⁴", "7.144×10⁻⁴"],
        ],
        note: "Pillow 基准。Probe A 用于估计，两个 Probe B 协议在估计与调参中都未出现。数值越低越好。参数最近邻 oracle 读取隐藏的目标参数，不是可部署基线。三次预先声明的目标划分中，局部岭的平均误差都低于最强的非 oracle 方法；更严格的逐目标非劣门槛通过两次，三次划分的非劣目标数分别是 3/5、4/5、5/5，六个划分—协议的 Probe-B 门槛全部通过。论文因此只声称稳定的平均优势，不声称逐目标全面占优。逐目标表里唯一的主协议例外是目标 (0.70, 1.20)：全局岭在参数误差与 B-幅值上更低，局部岭在 Probe A 与 B-方向上更低。",
      },
      { kind: "p", text: "渲染指标跟随动力学指标。六个 pillow 协议上，局部岭把平均前景 PSNR 从全局岭的 55.50 dB 提到 59.53 dB，物体裁剪 SSIM 从 0.9968 提到 0.9982。在留出的 standard_y 上看单个目标 002：局部岭 41.20 dB / 0.998，全局岭 32.93 dB / 0.977，响应 KNN 31.75 dB / 0.974，所示的 CMA-ES 固定种子 26.54 dB / 0.929，默认材料 19.41 dB / 0.892。论文说明由于形变相对静态篮筐很小，几种方法的渲染在视觉上都很接近，差异靠误差图定位在发生形变的坐垫上。这些 PSNR 与 SSIM 比较的是仿真器渲染的预测与仿真器渲染的目标，不衡量与真实视频的一致性。" },
      { kind: "p", text: "同一套流程在三个几何差异很大的资产上各建一套独立的响应库重跑，描述子与局部岭配置保持不变：局部岭在 Pillow 上 1.13%，Ficus 上 0.63%，Vasedeck 上 1.21%。Vasedeck 的 standard_x → standard_z 测试里，它的 Probe-B 轨迹 RMSE 是 1.37×10⁻⁵，最强的非 oracle 结果响应最近邻是 3.11×10⁻⁵，且在五个目标上全部非劣。Ficus 的 0.63% 对应 KNN 与全局岭的 3.71%、默认材料的 27.31%，平均前景 PSNR 41.87 dB 对全局岭的 26.80 dB。论文说明这是三个各自独立标定的物体特定回路，不是跨物体迁移。" },
      { kind: "fig", fig: { src: kgobjects, caption: "物体特定的材料标定。估计器形式相同，但每个高斯资产有自己的响应库。在 Pillow、Ficus 和 Vasedeck 上，局部岭都是最好的可部署方法。" } },
      { kind: "h", text: "观测阶梯" },
      { kind: "p", text: "上述结果用的都是仿真器导出的粒子状态，任何相机都给不出来。观测阶梯一次拆掉一层特权。前四档保持合成管线提供的精确或干净的几何对应关系，最后一档接近实用的图像跟踪器，并且打破了这种对应。" },
      {
        kind: "table",
        head: ["观测", "局部岭联合参数误差"],
        rows: [
        ...
        note: "最后一档上局部岭落后：全局岭 6.73%，响应 KNN 7.76%。论文因此写明当前瓶颈不只是内部填充粒子是否被观测到，稳健的对应关系与观测噪声都还没解决。对干净合成深度加噪，局部岭误差从 0.21% 升到噪声级 0.001 下的 2.65%、0.005 下的 16.25%、0.01 下的 28.26%。干净深度一档误差更低，是这个划分和这套特征构造下的经验结果，不能读成信息更少本身更好；这些数值也不构成对真实 RGB-D 性能的结论。",
      },
      { kind: "h", text: "契约边界" },
      { kind: "p", text: "响应库是在一份固定的、物体特定的仿真契约下建的。边界测试一次只改一个部件，先用原库评估失配，必要时再重建一份契约匹配的库。另外，五维描述子不是全局可辨识的：54 个候选构成的 1,431 个候选对里，有一些不同的参数对在描述子压缩之后彼此很近，局部邻域的条件数中位数是 1,244，最大 6,539。论文说明其中若干压缩空间里的歧义在考虑完整轨迹后可以分开，因此特征压缩本身贡献了额外的歧义；但这并不排除固定探针下的物理不可辨识。" },
      {
        kind: "table",
        head: ["条件变化", "局部岭联合参数误差"],
        rows: [
          ...
          ["同上，并归一化总施加力", "43.30%"],
          ["重建填充匹配的库", "0.99%"],
          ["改变 MPM 网格分辨率，沿用原库", "38.30%"],
          ["重建网格匹配的库", "0.75%"],
          ["同一填充规格下更换随机种子", "1.19%（最大 2.45%）"],
          ["留一物体、共享同一个估计器", "38–45%"],
        ],
        note: "归一化总施加力并不能修好填充失配，论文因此说明失效不能只归因于被选中的粒子数或总冲量。方法对随机填充实现稳健，但被建库时所用的物体与数值离散化条件所限定。",
      },
      { kind: "h", text: "与外部路线的对照" },
      { kind: "p", text: "论文把另外两条路线列为诊断而不是同信息排名，因为它们用的是不同的证据和不同的原生参数化。一是对单段 Probe-A 视频直接优化：CMA-ES 不使用粒子 ID，每个目标、每个种子 30 次前向仿真调用，三个预先声明的种子，五个目标共 450 次 rollout，参数冻结后在 standard_y 与 strong_x 上评测。它比默认参数好很多，联合参数误差 6.35% 对 27.31%，但落后于同一条合成深度观测轨道上的响应库局部标定 0.21%；Probe A 上局部标定 43.49 dB 对 CMA-ES 31.10 dB，两个 Probe B 协议上的排序一致。论文说明代价结构是互补的：本文摊销一个 54 次仿真的物体特定离线库，直接优化则按目标数付仿真调用。二是视觉先验与被动视频系统。PhysGM 观测 pillow 场景的四个静态视角，对五个视觉上完全相同的目标返回同一个材料预测，其共用适配器下的 Probe-A 响应误差是本文的 11.42 倍，并且在全部五个目标上都差于未标定的默认值。ReconPhys 观测被动视频并被校准到本文的候选区间，把联合参数误差降到 20.29%，在 4/5 个目标上优于默认响应，但 Probe-A 与未见 Probe-B 的视频误差仍约为本文的 2.6 倍与 2.9 倍。" },
      { kind: "h", text: "论文自陈的范围与局限" },
      { kind: "p", text: "论文把被支持的结论范围写得很窄：受控的、物体特定的、受离散化条件限定的两个 MPM 材料尺度标定，加上在同一仿真器族内对未见交互的预测。它不支持真实 RGB/RGB-D 输入、真实力反馈闭环、实测材料真值、sim-to-real 迁移、跨物体共享的估计器、填充未知条件下的反演、完整的多参数材料辨识、损伤预测，也不声称主动探针的最优性。所有候选与目标的参数和响应都由同一个仿真器生成，因此没有一项结果可以读作真实材料精度；观测阶梯用的是合成观测，外部路线用的是不同的原生参数化，被动可辨识性的结论只适用于被分析的那套弹簧—质点与接触公式。论文给出的下一步是：稳健的视觉对应、带不确定度的辨识、更丰富的物理状态与响应表示、主动交互选择，以及用实测的力与物体级形变来验证。" },
    ],
  },
